...
plt.style.use('default')  # Use default to align with light themes
sns.set_style("whitegrid", {
    "axes.facecolor": "#F0F2F6",    # Gray background inside plots
    "figure.facecolor": "#F0F2F6",  # Gray background outside plots
    "grid.color": "#DEDCD8"         # Light gray gridlines
})

# Customize plot colors to include 4 accent colors
plot_accent_colors = [
    "#4285F4",  # Blue
    "#DB4437",  # Red
    "#F4B400",  # Yellow
...
plt.rcParams['axes.prop_cycle'] = plt.cycler(color=plot_accent_colors)
plt.rcParams['axes.edgecolor'] = "#060606"   # Black for axis edges
plt.rcParams['axes.labelcolor'] = "#060606"  # Black for axis labels
plt.rcParams['text.color'] = "#060606"       # Black for general text
plt.rcParams['xtick.color'] = "#060606"      # Black for x-axis tick labels
plt.rcParams['ytick.color'] = "#060606"      # Black for y-axis tick labels
...
def load_data():
    ...
    df["Netflix_Hours"] = df["Netflix_Hours"].astype(flodf["Exercise_Frequency_Per_Week"] = df["Exercise_Frequency_Per_Week"].astype(float)n
    return df
"Exercise_Frequen
 habits and their academic performance in recent examninations")
